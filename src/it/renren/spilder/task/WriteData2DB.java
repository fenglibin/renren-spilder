...
public class WriteData2DB implements Task {
    ...
    public void doTask(ParentPage parentPageConfig, ChildPage childPageConfig, ChildPageDetail detail) throws Exception {
        if (dbo.getConn() == null) {
            dbo.initConn(parentPageConfig.getDatabase().getJdbcDriverClass(),
                         parentPageConfig.getDatabase().getLinkString(), parentPageConfig.getDatabase().getUsername(),
                         parentPageConfig.getDatabase().getPassword());
        }
        ...
        try {
            ...
            sql = "INSERT INTO renrenarctiny(typeid,channel,senddate,sortrank,mid) VALUES(" + tempTypeId
            ...
            sql = "INSERT INTO renrenarchives(id,typeid,ismake,channel,click,title,writer,source,pubdate,senddate,sortrank,mid,keywords,description,weight,dutyadmin,flag,litpic,filename) VALUES(?,"
                  ...
                  + "','"
                  + detail.getSource()
                  + "',unix_timestamp(now()),unix_timestamp(now()),unix_timestamp(now()),'1',?,?,?,1,?,?,?)";
            dbo.setSql(sql);
            ArrayList data = new ArrayList();
            data.add(Integer.parseInt(id));
            data.add(detail.getTitle().length() > 100 ? detail.getTitle().substring(0, 99) : detail.getTitle());
            data.add(detail.getKeywords().length() > 30 ? detail.getKeywords().substring(0, 29) : detail.getKeywords());
            data.add(detail.getDescription().length() > 255 ? detail.getDescription().substring(0, 254) : detail.getDescription());
            data.add(Integer.parseInt(id));
            data.add(flag);
            data.add(litpic);
