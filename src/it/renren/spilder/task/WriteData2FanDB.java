...
public class WriteData2FanDB implements Task {
    ...
    public void doTask(ParentPage parentPageConfig, ChildPage childPageConfig, ChildPageDetail detail) throws Exception {
        ...
        try {
            ...
            sql = "INSERT INTO renrenfanti_arctiny(typeid,channel,senddate,sortrank,mid) VALUES(" + tempTypeId
            ...
            sql = "INSERT INTO renrenfanti_archives(id,typeid,ismake,channel,click,title,writer,source,pubdate,senddate,sortrank,mid,keywords,description,weight,dutyadmin,flag,litpic,filename) VALUES(?,"
                  ...
                  + writer
                  + "','"
                  + source + "',unix_timestamp(now()),unix_timestamp(now()),unix_timestamp(now()),'1',?,?,?,1,?,?,?)";
            dbo.setSql(sql);
            data = new ArrayList();
            ...
            sql = "INSERT INTO renrenfanti_addonarticle (aid,typeid,body) VALUES(?," + typeid + ",?)";
            dbo.setSql(sql);
    ...
    private void initDbConn() {
        if (dbo.getConn() == null) {
            ...
            jdbcDriverClass = JDomUtil.getValueByXpath(taskDoc, "/Tasks/Task[@java='" + classname
                                                                + "']/Database/JdbcDriverClass/Value");
            linkString = JDomUtil.getValueByXpath(taskDoc, "/Tasks/Task[@java='" + classname
                                                           + "']/Database/LinkString/Value");
            username = JDomUtil.getValueByXpath(taskDoc, "/Tasks/Task[@java='" + classname
                                                         + "']/Database/Username/Value");
            password = JDomUtil.getValueByXpath(taskDoc, "/Tasks/Task[@java='" + classname
    ...
    private String jian2fan(String string) throws Exception {
        try {
            Document taskDoc = Main.getTaskDoc();
            String classname = this.getClass().getName();
            Element translater = (Element) XPath.selectSingleNode(taskDoc, "/Tasks/Task[@java='" + classname
                                                                           + "']/Translater");
