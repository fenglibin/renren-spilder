...
/**
 * 清洗原来获取的数据 
 * 如果原来的文章没有关键字，将标题设为关键字；如果原来的内容没有描述，将文章的前100个字符作为描述
	...
	public static void main(String[] args) throws SQLException {
		...
		try{
			for(time=startTime;time<times;time++){
				...
				ResultSet rs = st.executeQuery("select a.id,substring(b.body,1000) as body from renrenarchives a,renrenaddonarticle b where a.id=b.aid and a.keywords='' and a.id>"+start+" and a.id<="+end);
				String id = "";
				String sql="";				
				while(rs.next()){
					id = rs.getString("id");
