...
public class TranslatorUtil {
	 ...
	 * @throws Exception
	 */
	public static String translateHTML(String content, String fromLanguage,
	 ...
	 * @throws Exception
	 */
	public static String translateText(String content, String fromLanguage,
			String toLanguage) throws Exception {
		String[] contentArray = splitContentByLine(content);
		content = translate(contentArray, fromLanguage, toLanguage,true);
		return content;
	}
	private static String translate(String[] contentArray, String fromLanguage,
			String toLanguage,boolean lineSeperator) throws Exception {
	...
	private static Language getLanguage(String lang) {
		...
		else if (lang.equalsIgnoreCase("big5"))
	...
	private static String[] splitLargeContent(String content) {
		...
		if (content.indexOf(pstr) >= 0) {
			while (content.length() > Constants.GOOGLE_TRANSLATE_MAX_CHARATER_LENGTH) {
				String str1 = content.substring(0,
						Constants.GOOGLE_TRANSLATE_MAX_CHARATER_LENGTH);
	...
	private static int getLastLineSeprator(String content, String lineSeprator) {
		...
		if (content.indexOf(lineSeprator) > 0) {
			while (content.indexOf(lineSeprator, location
					+ lineSeprator.length()) > 0) {
				location = content.indexOf(lineSeprator, location
						+ lineSeprator.length());
			}
		} else {
