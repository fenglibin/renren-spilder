 ...
 * @version: 1.0.0 2010-5-5 下午08:37:48
 * 类说明:  
 */
import java.util.*;
import java.io.*;
...
public class PropertyManager {
  ...
  public void saveProperty() {
    ...
    try {
      ...
      log.error("There was an error writing properties to " + path + ". " +
          "Ensure that the path exists and that the cnjbb process has permission " +
                "to write to it -- " + ioe);
      // iolog4j.logError(e);
    }
    finally {
  ...
  public boolean propFileExists() {
    /*
     * String path = getProp("path", null); if (path == null) { return
     * false; }
     */
    File file = new File(this.path);
    if (file.isFile()) {
