...
class RemoteFaBu{
	...
	public static void doFaBu(int recordNum){
		try{
			...
			ResultSet rs = st.executeQuery("select top "+recordNum+" siteinfo.*,linktypes.typename,users.qq,users.email,users.password,users.regdate FROM (linktypes INNER JOIN siteinfo ON linktypes.id = siteinfo.sitetype) INNER JOIN users ON siteinfo.userid = users.id order by siteinfo.id desc");
			while(rs.next()){
				try{
