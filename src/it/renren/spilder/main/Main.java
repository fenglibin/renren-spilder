...
<script ]]></Value>
	...
	<Child>
		...
		<KeepFileName desc="是否保留原获取网页的文件名">
			<Value>
				<![CDATA[true]]>
			</Value>
		</KeepFileName>
...
public class Main {
			...
			if (args.length >= 1) {
				...
				if(dirOrFile.equals(Constants.EXECUTE_FILE)){
					if (!(new File(dirOrFileName).exists())) {
						System.err.println("Config File:" + dirOrFileName
								+ " not exists!");
						System.exit(0);
					}
					...
					if(oneFileSleepTime.equals("") && loopSleepTime.equals("")){
						...
						saveFromConfigDir(dirOrFileName, Long.parseLong(oneFileSleepTime), Long.parseLong(loopSleepTime));
					}
				}
			}
		} catch (Exception e) {			
	 ...
	 * @throws Exception
	 */
	private static void saveFromConfigDir(String configDirectory, long oneFileSleepTime,long loopSleepTime)	throws Exception {
		boolean loop=true;
		while (loop) {
	...
	private static void doSaveFromConfigDir(String configDirectory,long oneFileSleepTime) throws Exception {
		...
		if (!configFilePath.exists()) {
			System.err.println("Config Directory:" + configDirectory
					+ " not exists!");
			return;
		}
		//对当前目录进行处理
