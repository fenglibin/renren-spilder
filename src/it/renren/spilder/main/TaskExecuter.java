...
public class TaskExecuter extends Thread{
	...
	private static ParentPage initParentPage(Document ruleXml) throws JDOMException, EvalError{
		...
		parentPageConfig.getUrlListPages().setValues((Element) (XPath.selectSingleNode(ruleXml,"/Rules/MainUrl/Values")),Main.onePage);
		parentPageConfig.setImageDescUrl(JDomUtil.getValueByXpath(ruleXml,"/Rules/MainUrl/ImageDescUrl/Value"));
		parentPageConfig.setImageSaveLocation(JDomUtil.getValueByXpath(ruleXml,"/Rules/MainUrl/ImageSaveLocation/Value"));
		parentPageConfig.setRandRecommandFrequency(Integer.parseInt(JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/Recommend/Value").equals("") ? "0" : JDomUtil.getValueByXpath(ruleXml,"/Rules/MainUrl/Recommend/Value")));
		parentPageConfig.setSRcommand(JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/SRecommend/Value")==null ? false:Boolean.parseBoolean(JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/SRecommend/Value")));
		parentPageConfig.setFilterDownloadUrl(JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/FilterDownloadUrl/Value")==null ? true:Boolean.parseBoolean(JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/FilterDownloadUrl/Value")));
		...
		parentPageConfig.getUrlFilter().setCompByRegex(JDomUtil.getValueByXpath(ruleXml,"/Rules/MainUrl/UrlFilter/IsCompByRegex/Value")==null ? false:Boolean.parseBoolean(JDomUtil.getValueByXpath(ruleXml,"/Rules/MainUrl/UrlFilter/IsCompByRegex/Value")));
		parentPageConfig.setDesArticleId(JDomUtil.getValueByXpath(ruleXml,"/Rules/MainUrl/DesArticleId/Value"));
		parentPageConfig.setAutoDetectTypeMapClass(JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/AutoDetect/TypeMapMakeClass")==null ? "":JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/AutoDetect/TypeMapMakeClass"));
		parentPageConfig.getDatabase().setJdbcDriverClass(JDomUtil.getValueByXpath(ruleXml,"/Rules/MainUrl/Database/JdbcDriverClass/Value"));
		parentPageConfig.getDatabase().setLinkString(JDomUtil.getValueByXpath(ruleXml,"/Rules/MainUrl/Database/LinkString/Value"));
		parentPageConfig.getDatabase().setPassword(JDomUtil.getValueByXpath(ruleXml,"/Rules/MainUrl/Database/Username/Value"));
		parentPageConfig.getDatabase().setUsername(JDomUtil.getValueByXpath(ruleXml,"/Rules/MainUrl/Database/Password/Value"));
		parentPageConfig.setOneUrlSleepTime(JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/OneUrlSleepTime/Value")==null ? 0:Long.parseLong(JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/OneUrlSleepTime/Value")));		
		parentPageConfig.getTranslater().setFrom(JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/Translater/From/Value")==null ? "":JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/Translater/From/Value"));
		parentPageConfig.getTranslater().setTo(JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/Translater/To/Value")==null ? "":JDomUtil.getValueByXpath(ruleXml, "/Rules/MainUrl/Translater/To/Value"));
	...
	private static ChildPage initChildPage(Document ruleXml) throws JDOMException{
		...
		childPageConfig.getTitle().setIssRegularExpression(Boolean.parseBoolean(JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/Title/Replace/IsRegularExpression/Value")));
		...
		childPageConfig.getContent().setIssRegularExpression(Boolean.parseBoolean(JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/Content/Replace/IsRegularExpression/Value")));
		childPageConfig.getContent().setFrom(JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/Content/Replace/From/Value"));
		childPageConfig.getContent().setTo(JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/Content/Replace/To/Value"));
		childPageConfig.getContent().setWashContent(JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/Content/WashContent/Value")==null?"":JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/Content/WashContent/Value"));
		childPageConfig.getContent().setHandler(JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/Content/Handler/Value")==null?"":JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/Content/Handler/Value"));
		childPageConfig.setAddUrl(JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/AddUrl/Value")==null ? false : Boolean.parseBoolean(JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/AddUrl/Value")));
		childPageConfig.setKeepFileName(JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/KeepFileName/Value")==null ? false : Boolean.parseBoolean(JDomUtil.getValueByXpath(ruleXml,"/Rules/Child/KeepFileName/Value")));
		return childPageConfig;
	}
	private static void saveFromConfigFile(String configFile) throws Exception {		
		...
		try {
			...
			for (String listPageUrl : parentPageConfig.getUrlListPages().getListPages()) {
				String mainContent = HttpClientUtil.getGetResponseWithHttpClient(listPageUrl,parentPageConfig.getCharset());
				mainContent = getMainContent(mainContent, parentPageConfig);
				List<AHrefElement> childLinks = AHrefParser.ahrefParser(mainContent, parentPageConfig.getUrlFilter().getMustInclude(), parentPageConfig.getUrlFilter().getMustNotInclude(),parentPageConfig.getCharset(),parentPageConfig.getUrlFilter().isCompByRegex());
				...
				for (AHrefElement link : childLinks) {
					...
					try {
						...
						String childTitle = StringUtil.subString(childBody,childPageConfig.getTitle().getStart(),childPageConfig.getTitle().getEnd());
						...
						String keywords = MetaParser.getMetaContent(childBody,childPageConfig.getCharset(),Constants.META_KEYWORDS);
						...
						String description = MetaParser.getMetaContent(childBody, childPageConfig.getCharset(),Constants.META_DESCRIPTIONS);
						childBody = null;
						if(description.equals("") && description.length()>=Constants.CONTENT_LEAST_LENGTH){/*如果没有取得文章描述，就取文章内容的前100个字符为描述*/
							description = StringUtil.removeHtmlTags(childContent).trim().substring(0,Constants.CONTENT_LEAST_LENGTH);
						}
						...
						childContent = saveImages(childUrl, childContent,parentPageConfig.getImageDescUrl(),parentPageConfig.getImageSaveLocation(),childPageConfig.getCharset(), detail);
						if(childPageConfig.isAddUrl()){
							childContent = childContent + "<br>本文摘自："+detail.getUrl();
						}
						detail.setContent(childContent);
...
//						}
	...
	private static Task[] getTasks() throws JDOMException, IOException, InstantiationException, IllegalAccessException, ClassNotFoundException{
		Task[] allTask;
	 ...
	 * 
	 * @param url
	 *            当前图片页面所在的URL地址，或者是当前页面的主域地址；可以是"http://www.renren.it" 或
	 *            "http://www.renren.it/a/b.html"
	 * @param content
	 ...
	 * @throws Exception
	 */
	private static String saveImages(String url, String content, String imageDescUrl,
			String imageSaveLocation, String charset, ChildPageDetail detail)
			throws Exception {
		...
		List<ImageElement> imageElements = ImageParser.imageParser(content,
				charset);
